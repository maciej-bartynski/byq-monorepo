...
const unmarkBankAsPocket = async (values: {
    workspaceId: string;
    bankId: BankType['_id']
  }): Promise<any> => {
    ...
    const response = await fetchByQ(`workspace/${values.workspaceId}/bank-unpocket/${values.bankId}`, {
        method: 'PUT',
...
export default unmarkBankAsPocket
